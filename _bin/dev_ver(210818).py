...
parser.add_argument('--vis_thres', default=.6, type=float, help='visualization_threshold')
...
def anonymize_face_simple(image, factor=3.0):
    ...
    return cv2.GaussianBlur(image, (kW, kH), 10)
    # return cv2.blur(image, (kW, kH), 0)
...
if __name__ == '__main__':
    ...
    #VIDEO_FILE_PATH = "data/INHA/Guljahon.mp4"
    VIDEO_FILE_PATH = "elancer_test/Motion2-2.avi"
    OUTPUT_FILE = 'test_results/123.avi'
                ...
                if len(cur_list) > len(_list):
                    ...
                    for bb in cur_list:
                        ...
                        try:
                            ...
                            face = anonymize_face_simple(face, blocks=3)
                            ...
                            # text = "{:.4f}".format(b[4])
                            # b = list(map(int, b))
                            # cv2.rectangle(img_raw, (bb[0], bb[1]), (bb[2], bb[3]), (0, 0, 255), -1)
                            # cx = b[0]
                            # cy = b[1] + 12
                            # cv2.putText(img_raw, text, (cx, cy),
                            #             cv2.FONT_HERSHEY_DUPLEX, 0.5, (255, 255, 255))
