...
def make_blur(input_path, output_path, input_net, cfg, device):
...
def make_images(input_path, output_path):
    video_path = 'blurred_results/'

def sort_out_path_list(video_path):
    All_path = []
                            ...
                            for path5 in os.listdir(tmp_path4):
                                ...
                                for path6 in os.listdir(tmp_path5):
                                    ...
                                    for path7 in os.listdir(tmp_path6):
                                        ...
                                        # output = "results/"+target[len("examples"):-4]
                                        output = "blurred_results/"+target[len("../data/"):-4]
...
def main():
    ...
    for folder in folders:
        ...
        All_path = sort_out_path_list(video_path)


    for input_path, output_path in tqdm(All_path):

        tmp_output_path = output_path.split('/')
        tmp = tmp_output_path[:-1]
        tmp_next = '/'.join(tmp)
        ...
        if not os.path.exists(tmp_next):
            os.makedirs(tmp_next)

        make_blur(input_path, output_path, net, cfg, device)
